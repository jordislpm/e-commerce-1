...
function Card({product, title, id, category, price, description, image}:CardInfo) {
    ...
    const {setCount, count, setIsProductDetailOpen, setProductToShow}= useContextProducts()
  ...
  return (
    <div onClick={onDetail} className='bg-white cursor-pointer w-56 h-60 rounded-lg'>
        <figure className='relative mb-2 w-full h-4/5'>
            ...
            <div 
            className='absolute top-0 right-0 flex justify-center items-center bg-amazonOrange w-6 h-6 rounded-full m-2 p-1'
            onClick={addProduct}
            >
